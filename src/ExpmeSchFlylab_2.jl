...
function reached_charging_station(comb_state, flight_mode, charging_pos)
    ...
    if (norm(robot_pos - charging_pos) <= 0.05) && (flight_mode == "Landing")
        return true
    else
        return false
    end
...
function get_candidate_traj!(integrator, t_k, state_k, traj_nom_k, quad_key, q_num, discharge_rate)
    ...
    comp_time_b2b = @elapsed traj_b2b_kn = ExpController.DI_get_nominal_mpc_π(state_kN_end, integrator.p[q_num]; t0 = t_kN, T_H_sec = 9.0, N_mpc = 180, TRAJ = "b2b_mid", ren_point_1 = integrator.p[q_num].return_pos, ren_point_2 = integrator.p[q_num].charge_pos)
...
function simulate(quad_ic, t0, t_max, discharge_rate)
    ...
    # return params
    prob = ODEProblem(ExpDyn.N_closed_loop_quad!, quad_ic, tspan, params) 
