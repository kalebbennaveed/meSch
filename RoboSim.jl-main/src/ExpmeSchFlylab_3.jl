...
function reached_charging_station(comb_state, flight_mode, charging_pos)
    ...
    if (norm(robot_pos - charging_pos) <= 0.07) && (flight_mode == "Landing")
        return true
    else
        return false
    end
...
function recompute_u_track_cand!(integrator)
    ...
    t = integrator.t
    # Get the quad state
    quad_keys = keys(integrator.u)
    for i = 1:length(integrator.p)
        qi_state = integrator.u[quad_keys[i]] 
        # println("here1")
        at_charging = reached_charging_station(integrator.u[quad_keys[i]].x, integrator.p[i].flight_mode, integrator.p[i].charge_pos)
        # Check is it the first time quad has been reported to be at the charging station since it last charged
        if (integrator.p[i].landed == 0) && (at_charging == true)
            
            # Store the current time so that charging time can be tracked
            integrator.p[i].charge_start_time = t
            ...
            printstyled("cand_end_time: $(integrator.p[i].com_traj.ts[end])\n"; color = :blue)
            # end-1 because soc at the end is of the candidate trajectory which was not committed 
            printstyled("soc_at_end: $(integrator.p[i].can_soc_end[end-1])\n"; color = :blue)
            printstyled("-----------------------\n")
        ...
        if (at_charging == true)
            ...
            integrator.p[i].landed = 1
        end 
        # println("Going to controller")
        # println("here3")
        ui = ExpController.geometric_π_tracker(t, qi_state, integrator.p[i].com_traj; landing_status = integrator.p[i].landed, return_mode = integrator.p[i].return_mode, ren_point_1 = integrator.p[i].return_pos)
        # println("ui : $(ui)")
        integrator.p[i].u = ui
    end
end
...
function get_candidate_traj!(integrator, t_k, state_k, traj_nom_k, quad_key, q_num, discharge_rate)
    ...
    comp_time_b2b = @elapsed traj_b2b_kn = ExpController.DI_get_nominal_mpc_π(state_kN_end, integrator.p[q_num]; t0 = t_kN, T_H_sec = 10.0, N_mpc = 200, TRAJ = "b2b_mid", ren_point_1 = integrator.p[q_num].return_pos, ren_point_2 = integrator.p[q_num].charge_pos)
    push!(integrator.p[q_num].compute_time, comp_time_b2b)
    ...
    push!(integrator.p[q_num].cand_traj_time, cand_traj_duration)

    # Remember here traj_sol_cand_k and store the SoC at the end
    combined_state_sol = traj_cand_sol_k.u
    push!(integrator.p[q_num].can_soc_end, combined_state_sol[end].soc)

    # Store the candidate trajectory
    integrator.p[q_num].can_traj = traj_cand 
...
function recompute_committed_timed_quads!(integrator)
    ...
    for i = 1:length(integrator.p)
        ...
        charge_time_dur = 5.0
        return_time_dur = 2.0
        println("integrator.p[i].charge_start_time: $(integrator.p[i].charge_start_time)")
        println("charge_time_dur: $(charge_time_dur)")
        println("integrator.p[i].landed : $(integrator.p[i].landed )")

    # If the quad is done charging station, make sure to change the status of flight mode and landed status

        if (t_k >= integrator.p[i].charge_start_time + charge_time_dur) && (integrator.p[i].landed  == 1)

            integrator.p[i].return_mode = true
        end

        if (t_k >= integrator.p[i].charge_start_time + charge_time_dur + return_time_dur) && (integrator.p[i].landed  == 1)
            # Change the flight mode to Mission
            integrator.p[i].flight_mode = "Mission"

            # Change the landed mode to negative
            integrator.p[i].landed = 0

            # set the return mode to false
            integrator.p[i].return_mode = false
        ...
        # Update flight time
        flight_time = flight_time_estimation_cf_lin(integrator.u[quad_keys[i]], integrator.p[i].discharge_rate)
        push!(integrator.p[i].flight_time, flight_time)       
        ...
        if integrator.p[i].flight_mode != "Mission"
            continue
        end
        ...
        integrator.p[i].nom_traj = traj_nom_k

        ## Generate the candidate trajectory
        get_candidate_traj!(integrator, integrator.t, integrator.u[quad_keys[i]], integrator.p[i].nom_traj, quad_keys[i], i, integrator.p[i].discharge_rate)
    end


    ## meSch check ##
    quads_list = [quad for quad in integrator.p]
    ...
    if length(quads_list) > 1
        ...
        if gap_violation == true
            ...
            integrator.p[quad_LFT_id].flight_mode = "Landing"
            for i = length(quads_list):-1:2
                quad_id = quads_list[i].id
                integrator.p[quad_id].com_traj = integrator.p[quad_id].can_traj
            end               
        else
            printstyled("GAP FLAG: $(gap_violation)\n"; color = :green)
            ...
            for i = 1:length(quads_list)
                quad_id = quads_list[i].id
                if integrator.p[quad_id].can_soc_end[end] > 0. 
                    integrator.p[quad_id].com_traj = integrator.p[quad_id].can_traj
                else
                    integrator.p[quad_id].flight_mode = "Landing"
                end 
            end           
        end           
    ...
    elseif length(quads_list) == 1
        ...
        if integrator.p[quad_air_id].can_soc_end[end] > 0
            # candidate become committed {done}
            integrator.p[quad_air_id].com_traj = integrator.p[quad_air_id].can_traj
        else
            integrator.p[quad_air_id].flight_mode = "Landing"
        end
    else
        return 
    end
...
function simulate(quad_ic, t0, t_max, discharge_rate)
    tspan = (t0, t_max)
    ...
    quad_return_pos = [0.0; 0.0; 5.5]
    quad_keys = keys(quad_ic)
    ...
    quad1 = QuadObj(1,
                ...
                quad_charge_pos,
                quad_return_pos, 
                quad_ic[quad_keys[1]].soc, # initial soc
                discharge_rate,
                0.0,
                15.0,
                init_flight_mode,
    ...
    quad2 = QuadObj(2,
                ...
                quad_charge_pos,
                quad_return_pos, 
                quad_ic[quad_keys[2]].soc, # initial soc
                discharge_rate,
                0.0,
                15.0,
                init_flight_mode,
    ...
    quad3 = QuadObj(3,
                ...
                quad_charge_pos,
                quad_return_pos, 
                quad_ic[quad_keys[3]].soc, # initial soc
                discharge_rate,
                0.0,
                15.0,
                init_flight_mode,
    ...
    # return params
    prob = ODEProblem(ExpDyn.N3_closed_loop_quad!, quad_ic, tspan, params) 
    
    # Callbacks
    
    # Committed trajectories for the Quad
    cb_traj_com_quads = PeriodicCallback(recompute_committed_timed_quads!, 1.5; initial_affect=true)
